...
from neuralprocesses.dist.uniform import UniformContinuous, UniformDiscrete
...
def get_batches(
    num_context: int, num_batches: int, gen_type: str, target_size: int, config: dict
):
...
if __name__ == "__main__":
    ...
    for dim_y, gen_type, target_size in itertools.product(DIM_Y, GEN_TYPE, TARGET_SIZE):
        ...
        torch.save(
            batches,
            f"benchmark_dataset_{target_size}_targets_{gen_type}_{dim_y}_layers.pt",
        )
